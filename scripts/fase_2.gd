...
func _ready():
	# A parte do reservatório continua igual
	...
	# Agora, este loop SÓ vai passar pelos nós corretos (seus OpcaoGasto),
	# ignorando completamente as Sprites!
	for opcao in todas_as_opcoes:
		# Conecta o sinal 'toggled' de cada opção à nossa função de lógica.
		# (Use 'pressed' se você estiver usando um Button normal, ou 'estado_mudou' para nossa versão customizada)
